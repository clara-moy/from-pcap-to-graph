...
for ip in ipv4[router]:
    ...
    graphs_wan[ntwk_index].add_edge(router, list_nodes_wan[ntwk_index].index(ip))
...
for index in range(len(graphs_wan)):
    ...
    graphs_renderers_wan[ntwk_index] = create_layout(
        graphs_wan[ntwk_index], router, (column + 2.2, index % dim - 1.3), scale=0.1
    )
